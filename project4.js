new Vue({
    ...
    computed: {
        selectedTerms: function () {
            ...
            searchOptions = searchOptions.trim().toLowerCase();

            terms_array = terms_array.filter(function(item){
                if(item.title.toLowerCase().indexOf(searchOptions) !== -1){
                    return item;
                }
            })
